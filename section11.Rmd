...
0. Today is my daughter's first birthday!
1. Problem set 2 is graded. Two common stumbling blocks:
    - When comparing a model that allows for treatment-effect heterogeneity for each of five groups: How many assumptions are in $H_o: \beta_1 = \beta_2 = \beta_3 = \beta_4 = \beta_5$? Four. We impose this hypothesis when we assume no heterogeneity (_i.e._, only $\beta$---no $\beta_i$s).
    - Can you use OLS residuals to verify the strict exogeneity? No: strict exogeneity is an assumption about the unobserved disturbances ($\varepsilon_i$)---not the the OLS residuals ($e_i$).
2. New problem set due today.
3. If you are interested in learning more about using R for geospatial analyses/data work, the Geospatial Innovation Facility (GIF) has some nice resources and [workshops](http://gif.berkeley.edu/support/workshop_descriptions.html#geor).
...
What happens when we calculate cluster-robust standard errors—standard errors that allow for intra-cluster correlation of the disturbances—when in reality the disturbances are independent?^[A similar situation: what happens when we cluster too conservatively?] I wrote up some code to simulate this scenario. _Note_: I use a new package here (`data.table`). It is one of my go-to packages, but it has a syntax that is a bit different from all the packages we've covered thus far. Check out the [vignette](https://cran.r-project.org/web/packages/data.table/vignettes/datatable-intro.html) or the [Github repository](https://github.com/Rdatatable/data.table/wiki/Getting-started)—they are quite helpful. In addition, Hadley Wickham is working on a package [`dtplyr`](https://github.com/hadley/dtplyr) that provides a "data.table backend for dplyr".
...
One more case for comparison: Let's see the same graphs when there is within-cluster covariance in the disturbances.

```{R, simulation 2 setup}
# Simulation parameters ----
# Observations per group
n_g <- 30
# Number of groups
...
Why wouldn't the researcher observe $x_2$—an obviously important variable? One reason is that we do not always know which variables determine $y$ and correlate with other covariates. Another reason is that we do not always get all the data that we want.^[Or the governmental department that has the data that you want drags its feet for three years and then gives you an unusably aggregated version of the data that you actually wanted that covers two whole years and is somehow still missing half of its observations.] In either situation, we eventually end up with some variables in the regression and some variables left out.
...
Thus, our covariate is correlated with our disturbance, violating population orthogonality. What's the big deal? We can no longer assume the OLS estimator for $\boldsymbol{\beta}$ is consistent. Not good, my friends.

For a slightly different view: recall from our [section on the Frisch-Waugh-Lovell theorem](section04.html) that when we regress $y$ on $x_1$ and $x_2$, the estimate for $\beta_1$ (the coefficient on $x_1$) is

$$ b_1 = \left(\mathbf{X}_1'\mathbf{X}_1\right)^{-1}\mathbf{X}_1'\mathbf{y} - \left(\mathbf{X}_1'\mathbf{X}_1\right)^{-1}\mathbf{X}_1'\mathbf{X}_2 b_2 $$

The second term in the difference reminds us that we will calculate a different (wrong/inconsistent) coefficient if we omit $x_2$ when (1) $x_1$ and $x_2$ have non-zero covariance (_i.e._, $\mathbf{X}_1'\mathbf{X}_2 \neq \mathbf{0}$) and (2) $y$ and $x_2$ have non-zero covariance (_e.g._, $b_2 \neq 0$). More succinctly: if $x_1$ and $x_2$ are not orthogonal, and if $y$ and $x_2$ are not orthogonal, then if you omit $x_2$ from your regression of $y$ and $x_1$, your estimate for $\beta_1$ is inconsistent/biased. The inconsistency/bias comes from the effect of $x_2$ "loading" onto $x_1$ when you fail to explicitly control for $x_2$.
...
In the _first stage_ of 2SLS, we regress our covariates on the instruments (again, non-endogenous covariates are their own instruments). Returning to the case where $x$ is endogenous, the first stage looks like:

$$ x = \gamma_0 + \gamma_1 z + u $$

We then use the fitted values from the first stage—_i.e._, $\hat{x} = \hat{\gamma}_0 + \hat{\gamma}_1 z$—as the covariates in the second stage:

$$ y = \beta_0 + \beta_1 \hat{x} + v $$

If we have a valid instrument, then our estimate of $\beta_1$ in the second stage is consistent for $\beta_1$.

So what is going on here? The first stage is purging the "bad variation" in $x$. By regressing $x$ on a variable that is uncorrelated with our omitted variable(s), we are keeping only the variation in $x$ that is exogenous—uncorrelated with the error term. Then, when we use this purged version of $x$ ($\hat{x}$) in the second stage, we no longer violate population orthogonality. Consistent estimates!
...
1. You use a nonlinear predictor in your first stage, _e.g._, probit, logit, Poisson, _etc_. You need linear OLS in the first stage to guarantee that the covariates and fitted values in second stage will be uncorrelated with the error (exogenous).
...
While 2SLS is great when combined with experiments, most of us won't be using it strictly in a randomized experimental setting. In these non-experimental settings, you need to be even more careful when implementing 2SLS/instrumental variables. The exclusion restriction is very important and untestable. One way to provide some evidence of the legitimacy of your 2SLS estimates is to show the OLS (non-instrumented) regression alongside the 2SLS (instrumented) regression. If there is indeed omitted variable bias (or simultaneity)—and if your instrument has some legitimacy—then the the OLS and 2SLS estimates will differ. If the difference is consistent with some economic reasoning on the direction of the bias, then you have more plausibility.

In short: instrumental variables/2SLS requires some humility, creativity, and caution.

## Measurement error

You might already be aware of this fact: data are not always perfect. In fact, when we start delving deep into our data, it is often frightening how messy they are. It turns out that one of the original implementations of IV was as a solution to measurement error. In this case, the exclusion restriction is often a bit less difficult to swallow, and the first stage is generally quite strong.
