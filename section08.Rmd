...
The `between()` function (in `dplyr`) gives you a shortcut to `(x >= left) & (x <= right)`. Its syntax: `between(x, left, right)`, _e.g._,
```{R between}
# Define x
x <- 3
# The "normal" way
...
```{R long_run_sim, message = F, cache = T, results = "hide"}
# Set the seed (again)
set.seed(12345)
...
```{R arrange_four_plots, fig.width = 8, fig.height = 8}
# 'Join' the plots
gg_sim <- arrangeGrob(
...
```{R pareto_10k, message = F, cache = T, results = "hide"}
par_10k <- run_par(
  n      = 1e4,
  n_iter = 1e4,
  data   = par_df)
...
```{R sim_pareto_100k, message = F, cache = T, results = "hide"}
par_100k <- run_par(
  n      = 1e5,
  n_iter = 1e4,
  data   = par_df)
