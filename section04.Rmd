...
_Reminder_: Office hours today from 1pm–3pm in Giannini 254.
...
If you want to know how to save plots produced by the `plot()` function, see below. Soon, we will cover a function that I prefer to `plot()` (the function is from the package `ggplot2`), which has a different—easier—saving syntax. Nevertheless, here is the answer to saving a `plot()`.
...
Note that we are using `select_()` here (with the `.dots` argument), rather than plain old `select()`. `select_(.dots = ...)` allows us to feed it a character vector of names.
...
Here, BIC (aka SIC) appears to do the best job noticing when the data become poor.
